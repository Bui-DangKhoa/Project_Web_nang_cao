...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    const productTitle = `${product.name} | VELOUR Shop`;
    ...
    upsertMeta("description", productDesc);
    upsertMeta("keywords", keywords);

    upsertMeta("og:type", "product", "property");
    upsertMeta("og:title", productTitle, "property");
    upsertMeta("og:description", productDesc, "property");
    upsertMeta("og:image", productImage, "property");
    upsertMeta("og:url", productUrl, "property");

    upsertMeta(
      "twitter:card",
      productImage ? "summary_large_image" : "summary",
    );
    upsertMeta("twitter:title", productTitle);
    upsertMeta("twitter:description", productDesc);
    upsertMeta("twitter:image", productImage);

    upsertCanonical(productUrl);
  }, [product]);

  // Kiểm tra sản phẩm đã nằm trong wishlist của user chưa
  useEffect(() => {
  ...
  const handleToggleWishlist = async () => {
    if (!user) {
      showToast("Vui lòng đăng nhập để sử dụng yêu thích");
      navigate("/login");
      return;
    }
    try {
      ...
      const msg =
        err.response?.data?.message || "Không thể cập nhật danh sách yêu thích";
      showToast(msg);
    } finally {
        ...
        <div>
          ...
          {product.originalPrice > product.price && (
            <div className="detail-original">
              ...
              <span
                style={{ color: "var(--red)", marginLeft: 8, fontSize: 14 }}
              >
                -{discount}%
              </span>
            </div>
            ...
            <>
              ...
              <div className="size-options">
                {product.sizes.map((s, i) => (
                  <button
                    key={i}
                    className={`size-btn ${selectedSize === i ? "selected" : ""}`}
                    onClick={() => setSize(i)}
                  >
                    {s}
                  </button>
